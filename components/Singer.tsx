    ...
    return (
        <div className={`select-none w-full flex flex-col gap-7 items-center justify-center px-2 ${props.isFav && 'bg-pattern-2 rounded-boxRadius py-4'}`}>
            ...
            >
                <Image
                    data-key={props.id}
                    className={`cursor-pointer ${props.isFav ? 'rounded-singerFav' : 'rounded-full hover:scale-105 active:scale-95'} border border-rose-500 shadow-musicShadow border-dotted hover:brightness-125 transition-all duration-300`}
                    src={props.image || ''}
                    alt={props.name}
                    width={500}
                    height={500}
